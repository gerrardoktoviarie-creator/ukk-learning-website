...
// Fetch statistics
$mapel_query = "SELECT COUNT(*) as total FROM mapel";
$mapel_result = $conn->query($mapel_query);
$mapel_count = $mapel_result->fetch_assoc()['total'];

// Get user's enrolled mapels
$enrolled_query = "SELECT COUNT(*) as total FROM user_mapel WHERE user_id = {$user['id']}";
$enrolled_result = $conn->query($enrolled_query);
$enrolled_count = $enrolled_result->fetch_assoc()['total'];

// Get all mapels with enrollment status
$mapel_list_query = "SELECT m.*, 
                    (SELECT COUNT(*) FROM user_mapel WHERE user_id = {$user['id']} AND mapel_id = m.id) as enrolled
                    FROM mapel m 
                    ORDER BY m.id ASC";
$mapel_list_result = $conn->query($mapel_list_query);
$mapels = [];
if ($mapel_list_result) {
    while ($row = $mapel_list_result->fetch_assoc()) {
        $mapels[] = $row;
    }
}
?>
...
<body>
    ...
    <div class="dashboard-container">
        ...
        <div class="dashboard-header">
            <h1><i class="fas fa-tachometer-alt" style="color: #667eea;"></i> Dashboard Pembelajaran</h1>
        ...
        <div class="stats-grid">
            <div class="stat-card">
                ...
                <div class="stat-number"><?php echo $mapel_count; ?></div>
            ...
            <div class="stat-card">
                ...
                <div class="stat-label">Mapel Diikuti</div>
            </div>
            
            <div class="stat-card">
                <div class="stat-icon">🎯</div>
                <div class="stat-number"><?php echo $mapel_count > 0 ? round(($enrolled_count / $mapel_count) * 100) : 0; ?>%</div>
        ...
        <!-- Mapel Grid -->
        <h2 style="color: #2E86AB; margin-bottom: 30px; font-size: 24px;">
            <i class="fas fa-book-open"></i> Daftar Mata Pelajaran
        </h2>
        ...
        <div class="mapel-grid" id="mapelGrid">
            <?php foreach ($mapels as $mapel): ?>
                <div class="mapel-card" data-mapel="<?php echo htmlspecialchars(strtolower($mapel['nama_mapel'])); ?>">
                    ...
                    <div class="mapel-status <?php echo $mapel['enrolled'] ? '' : 'not-enrolled'; ?>">
                        <?php echo $mapel['enrolled'] ? '✓ Sedang Diikuti' : '+ Belum Diikuti'; ?>
                    </div>
                    <div class="mapel-description"><?php echo htmlspecialchars(substr($mapel['deskripsi'], 0, 80)); ?>...</div>
                    <div class="mapel-teacher">
                        <i class="fas fa-chalkboard-user"></i> <?php echo htmlspecialchars($mapel['guru_pengampu']); ?>
                    </div>
                    <a href="mapel.php?id=<?php echo $mapel['id']; ?>" class="btn-mapel">
    ...
    <script>
        ...
        document.querySelectorAll('.btn-mapel').forEach(button => {
